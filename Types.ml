...
let rec type_of_expr expr env =
  ...
  and
    (* ...............A COMPLETER .......................................*)
    ruleFunction par body env = let newVar = newVariable() in FunctionType (newVar, type_of_expr body ((par,newVar)::env))

  and 
    (* ...............A COMPLETER .......................................*)
    ruleCall fct par env = 
  let fctType = (type_of_expr fct env) in
    let parTypeR = (type_of_expr par env) in
      let parType = newVariable () in
        let bodyType = newVariable () in
           let _, isOk1 = unify fctType (FunctionType(parType, bodyType)) in
           let _, isOK2 = unify parTypeR parType in
              (if (isOk1 && isOK2) then bodyType else ErrorType)
  ...
  and 
    (* ...............A COMPLETER .......................................*)
    ruleRef expr env = let typeex = type_of_expr expr env in 
    let (_, isOk1) = unify typeex ErrorType in
    let (_, isOk2) = unify typeex UnknownType in
    if (isOk1 || isOk2) then ErrorType
    else ReferenceType(typeex)
  and
    (* ...............A COMPLETER .......................................*)
   ruleRead expr env = 
  let exprType = type_of_expr expr env in
  let ref = newVariable() in
  let (_, isOk) = unify exprType (ReferenceType(ref)) in
  if isOk then ref else ErrorType 

  and
    (* ...............A COMPLETER .......................................*)
    ruleWrite left right env =   let typeL = type_of_expr left env in 

  let typeR = (type_of_expr right env) in 
  let ref = newVariable() in
  let (_, isOk1) = unify typeL (ReferenceType(ref)) in
  let (_, isOk2) = unify typeR ref in
  
  if isOk1 && isOk2 then UnitType else ErrorType

  and
    (* ...............A COMPLETER .......................................*)
    ruleSequence left right env =  
    let typeLeft = type_of_expr left env in 
        let (_,isOK) = unify typeLeft UnitType in
        if isOK then type_of_expr right env else  ErrorType

  and
    (* ...............A COMPLETER .......................................*)
    ruleWhile cond body env = 
    let typeCond = (type_of_expr cond env) in
    let typeBody = (type_of_expr body env) in
    let _, isOk1 = unify typeCond BooleanType in
    let resultType, isOK2 = unify typeBody UnitType in
    (if (isOk1 && isOK2) then resultType  else ErrorType)
