...
let rec lookforMem name mem =
  match mem with
    | [] -> NotFound
    | (key,value) :: others ->
      (if (key = name) then (Found value) else (lookforMem name others));;

let rec writeInMem ref value mem =
  match mem with
    |[]-> assert false
    |(r,v)::qmem-> if r =ref then (ref,value)::qmem else (r,v)::(writeInMem ref value qmem)
...
ruleWrite refexpr valexpr mem env = 
  (let (vrefexpr,mrefexpr) = (value_of_expr (refexpr,mem) env) in
     (match vrefexpr with
     | (ErrorValue _) as result -> (result,mrefexpr)
     | ReferenceValue reference ->
       (match (lookforMem reference mrefexpr) with
       | NotFound -> ((ErrorValue (UnknownReferenceError reference)),mrefexpr)
       | Found value -> 
         (let (vvalexpr,mvalexpr) = 
            (value_of_expr (valexpr,mrefexpr) env) 
          in
            (match vvalexpr with
            | (ErrorValue _) as result -> (result,mvalexpr)
            | _ -> (NullValue,((reference,vvalexpr)::mvalexpr)))))
     | _ -> ((ErrorValue TypeMismatchError),mrefexpr)))
...
ruleSequence left right mem env = 
  (let (leftvalue,leftmem) = 
     (value_of_expr (left,mem) env) 
   in
     (match leftvalue with
     | (ErrorValue _) as result -> (result,leftmem)
...
(let (cval,cmem) = 
   ...
   in
     ...
     | (BooleanValue rcond) ->
       (if (rcond) then 
          let vbody, mbody = value_of_expr (body,mem) env in
          ruleWhile cond body mbody env 
       else 
          (NullValue,cmem))
     | (ErrorValue _) as result -> (result,cmem)
     | _ -> ((ErrorValue TypeMismatchError),cmem)))
...
ruleReference expr mem env = let vexpr,mexpr = value_of_expr (expr,mem) env in
match vexpr with
|(ErrorValue _) as result -> (result,mexpr)
|_-> let reference = newReference() in ((ReferenceValue reference), (reference,vexpr)::mexpr)
(* ...............A COMPLETER .......................................*)
